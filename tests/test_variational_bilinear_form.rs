...
impl<Mon:Monomial,MeshT:Mesh<Mon>> VariationalBilinearForm<Mon,MeshT> for AsymmetricTestVBF<Mon,MeshT> {
...
impl<Mon:Monomial,MeshT:Mesh<Mon>> VariationalBilinearForm<Mon,MeshT> for SymmetricTestVBF<Mon,MeshT> {
...
fn test_asymmetric() {
  ...
  let vbf: AsymmetricTestVBF<Mon2d,RectMesh<Mon2d>> = AsymmetricTestVBF;

  let m = vbf.basis_els_vs_basis_els_transpose(basis);
    ...
    for c in range(0, basis.num_els()) {
      ...
      if basis.is_int_supported(r) {
        ...
        if basis.is_int_supported(c) { // (r,c) is an int vs int el pair
          ...
          let ip = if r_int_fe == c_int_fe { vbf.int_mon_vs_int_mon(OShape(0), c_monn, r_monn, basis) } else { 0 as R };
          ...
          let ip = if c_incls.fe1 == r_int_fe || c_incls.fe2 == r_int_fe {
            ...
            vbf.side_mon_vs_int_mon(OShape(0), c_monn, c_sf, r_monn, basis)
          } else { 0 as R };
          assert_eq!(m.get(*r,*c), ip);
        }
      }
        ...
        if basis.is_int_supported(c) { // (r,c) is an side vs int el pair
          ...
          let ip = if r_incls.fe1 == c_int_fe || r_incls.fe2 == c_int_fe {
            ...
            vbf.int_mon_vs_side_mon(OShape(0), c_monn, r_monn, r_sf, basis)
          } else { 0 as R };
          assert_eq!(m.get(*r,*c), ip);
        }
          ...
          if r_incls.fe1 == c_incls.fe1 {
            ip += vbf.side_mon_vs_side_mon_fe_contr(OShape(0), c_monn, c_incls.side_face_in_fe1, r_monn, r_incls.side_face_in_fe1, basis);
          } 
          else if r_incls.fe1 == c_incls.fe2 {
            ip += vbf.side_mon_vs_side_mon_fe_contr(OShape(0), c_monn, c_incls.side_face_in_fe2, r_monn, r_incls.side_face_in_fe1, basis);
          }
          
          if r_incls.fe2 == c_incls.fe1 {
            ip += vbf.side_mon_vs_side_mon_fe_contr(OShape(0), c_monn, c_incls.side_face_in_fe1, r_monn, r_incls.side_face_in_fe2, basis);
          } 
          else if r_incls.fe2 == c_incls.fe2 {
            ip += vbf.side_mon_vs_side_mon_fe_contr(OShape(0), c_monn, c_incls.side_face_in_fe2, r_monn, r_incls.side_face_in_fe2, basis);
          }
          assert_eq!(m.get(*r, *c), ip);
        }
      }
...
fn test_symmetric() {
  ...
  let vbf: SymmetricTestVBF<Mon2d,RectMesh<Mon2d>> = SymmetricTestVBF;
    ...
    for c in range(0, basis.num_els()) {
      ...
      if basis.is_int_supported(r) {
        ...
        if basis.is_int_supported(c) { // (r,c) is an int vs int el pair
          let c_int_fe = basis.support_int_fe_num(c);
          let c_monn = basis.int_rel_mon_num(c);
          let ip = if r <= c && r_int_fe == c_int_fe { vbf.int_mon_vs_int_mon(OShape(0), c_monn, r_monn, basis) } else { 0 as R };
          assert_eq!(m.get(*r,*c), ip);
        }
        else { // (r,c) is an int vs side el pair
          let c_incls = basis.fe_inclusions_of_side_support(c);
