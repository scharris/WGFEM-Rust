...
\begin{frame}
  ...
  \pause
  \begin{itemize}[<+->]
    \item VBF's will be implemented by supplying only the per-element contribution function \spot<2->{$\phi$}.
    ...
    \item Final VBF ($\phi$) interface will be\\
      \begin{itemize}[<+->]
        ...
        \item \texttt{\small \textcolor{blue}{side\_mon\_vs\_side\_mon(oshape, smonn1, sf1, smonn2, sf2)}}
        \item \texttt{\small \textcolor{blue}{is\_symmetric() }}
      \end{itemize}
  \end{itemize}
\end{frame}

% TODO: Move this to the end, with the mesh implementations, too distracting here.
\begin{frame}
...
\begin{frame}
  ...
  \texttt{\small \textcolor{blue}{side\_mon\_vs\_side\_mon(oshape, smonn1, sf1, smonn2, sf2)}}\\
  \vspace{.3cm}
  
  \pause
  \uncover<+-> {
  -- The first term is computed as before, using basis function 
     {\small \texttt{\textcolor{blue}{basis.\textbf{\textcolor{orange}{side\_mon\_wgrad}(monn, oshape, side\_face)}}}}\\
     to obtain the weak gradients and the same mesh function as before to perform the integration.\\
 
  \vspace{.2cm}
  \uncover<+-> {
  -- For the stabilization term, the interior pieces to be projected are 0, so the $Q_b$ values vanish, leaving
    \begin{align*}
      s(v,w) &= h_T^{-1} \langle -v, -w \rangle_{\partial T} = h_T^{-1} \sum_{s \in sides(T)} \langle v, w \rangle_s\\
             &= 
            \begin{cases}
              h_T^{-1} \langle v, w \rangle_s \quad &\mbox{ if $v$ and $w$ have common support side $s$}\\
              0, &\mbox{ otherwise}
            \end{cases}
    \end{align*}
  }}
...
%TODO: is_symmetric
%\item \texttt{\small \textcolor{blue}{is\_symmetric() }}
%      Yes if \textbf{$a$} matrix is provided, else no.  Always OK to answer no, though yes answer enables more efficient algorithms.
% laplace vbf contd
% int\_mon\_vs\_int\_mon contd
%  Second term:
%    Introduce new mesh function to fetch maximum element diameter, $h_T$.
%    etc
%  $$\mathfrak a_s(v,w) = \sum_{T \in M} \left\{ (a \nabla_w v,\nabla_w w)_{\scriptscriptstyle T_0} \;+\;
%    h_T^{-1}\langle Q_b v_{\scriptscriptstyle T_0} - v,Q_b w_{\scriptscriptstyle T_0} - w \rangle_{\partial T} \right\}$$
