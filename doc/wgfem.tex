...
\begin{frame}
  ...
  \frame{\includegraphics[width=12cm, height=2cm]{img/basis_layout_overall.pdf}}
  \pause
...
\begin{frame}
  ...
  \frame{\includegraphics[width=12cm, height=.73cm]{img/basis_layout_overall_thin.pdf}}
  \pause
  After querying using {\small \texttt{\textcolor{blue}{is\_int\_supported(i), is\_side\_supported(i)}}}, we can then use the following
   functions to translate from the global basis element number to the supporting interior or non-boundary side number, and shape function monomial: 
  \pause
  \begin{align*}
...
\begin{frame}
  ...
  \begin{align*}
    ...
    \text{\texttt{\textcolor{blue}{fe\_side\_mon\_el\_num({fe, side\_face, monn})}}} &= s_0 + n \cdot n_s + monn\\
      \text{\scriptsize where } \text{\scriptsize \textcolor{blue}{n}} = 
      \text{\scriptsize \textcolor{blue}{mesh.\textcolor{orange}{nb\_side\_num\_for\_fe\_side}(fe, side\_face) }} & \text{\scriptsize in the latter}.
  \end{align*}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item weak gradient solver -- basic ideas, basis of vector monomials, basic howto.
      Weak Gradient
        Discuss vector monomial basis spanning space of vector polynomials of deg k-1.
        Include discussion from wgrad module.
        Efficiency
          Can reduce the work to only one system solve per oshape, because the vector monomial basis vs basis matrix is the same so long as we
          stay on the same oshape, so we can stack multiple rhs's together into the same solve.  Each rhs is a face monomial to have its weak
          gradient solved for. (Show that solving matrix * matrix = matrix is equiv to multiple solves of matrix * vector = vector).
            wgrads_on_oshape<MESHT:Mesh<Mon>>(&mut self, int_mons: &[Mon], side_mons_by_side: &[&[Mon]],
