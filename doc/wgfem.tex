...
\begin{frame}
  ...
  \texttt{\textbf{for} \textcolor{blue}{nbs\_1} in \{1,$\dots$,\textcolor{blue}{mesh.\textcolor{orange}{num\_nb\_sides}()}\}\\
    \hspace{0.4cm}\textbf{for} \textcolor{blue}{inter} in \textcolor{blue}{\large \textbf{asc\_nb\_sides\_interactions}(nbs\_1)}\\
    \hspace{0.8cm}\textbf{for} \textcolor{blue}{monn\_1} in 
                     \{1,$\dots$,\textcolor{blue}{basis.\textcolor{orange}{mons\_per\_fe\_side}()}\}\\
    \hspace{0.8cm}let \textcolor{blue}{i} = \textcolor{blue}{basis.\textcolor{orange}{nb\_side\_mon\_el\_num}(nbs\_1, monn\_1)}\\
  } 

  \texttt{
    \hspace{1.2cm}if \textcolor{blue}{inter} is a single-fe interaction,\\
      \hspace{1.8cm}say \textcolor{blue}{inter} = (fe, nbs\_1\_sf\_in\_fe, nbs\_2\_sf\_in\_fe)\\
    \hspace{1.2cm}let fe\_oshape = mesh.oriented\_shape\_for\_fe(fe)\\
    \hspace{1.2cm}for monn\_2 in range(0, num\_side\_mons)\\
    \hspace{1.6cm}let c = basis.nb\_side\_mon\_el\_num(nbs\_2, monn\_2)\\
    \hspace{1.6cm}let ip = self.get\_side\_vs\_side\_vbf\_contr(fe\_oshape, nbs\_2\_sf\_in\_fe, nbs\_1\_sf\_in\_fe, monn\_2, monn\_1,
                                                  side\_vs\_side\_vbf\_fe\_contrs)\\
    \hspace{1.6cm}m.push(r, c, ip);
  }
\end{frame}


% TODO: Maybe put this just before the actual side-side algorithm.
%(Implications of two-fe interaction possibility in side-side interactions)
%\pause
  %\vspace{.15cm}
  %This has two important implications for side-side interactions:
  %\begin{enumerate}[<+->]
    %\item We will have to sum the $\phi$ contributions to the total vbf $\mathfrak{a}(v,w) = \sum_T { \phi(v|_T,w|_T) }$ from up to
      %two finite elements.
    %\item When iterating sides we need to use the global non-boundary side numbers to ensure that each side is only visited once,
      %because this numbering is 1-1 with the non-boundary sides, unlike local reprs.
  %\end{enumerate}
