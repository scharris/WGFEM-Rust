...
fn solve_laplace<'a, Mon:Monomial, MeshT:Mesh<Mon>>
  ...
  let boundary_projs = &boundary_projections(g, basis);

  let sys_matrix = vbf.basis_els_vs_basis_els_transpose();

  let sys_rhs = DenseMatrix::from_fn(basis.num_els(), 1, |i,_| 
    ip_on_ints(|x|f(x), BasisElNum(i), basis)
    - 
    vbf_boundary_projs_vs_bel(vbf, boundary_projs, BasisElNum(i), basis)
  );
  
/* TODO: Do sparse solve here.
...
fn vbf_boundary_projs_vs_bel<'a, Mon:Monomial, MeshT: Mesh<Mon>>
   (vbf: &VBFLaplace<'a,Mon,MeshT>,
    boundary_projs: &BoundaryProjections<'a,Mon>,
    bel: BasisElNum,
    basis: &WgBasis<Mon,MeshT>) -> R {
  ...
  if basis.is_int_supported(bel) {
    ...
    range(0, mesh.num_side_faces_for_shape(fe_oshape)).fold(0 as R, |sum, sf| {
        match boundary_projs.find(&(fe, SideFace(sf))) {
          Some(bnd_proj) => { 
